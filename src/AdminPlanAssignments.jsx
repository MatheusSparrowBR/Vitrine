...
export default function AdminPlanAssignments({supabase,onToast}){
 ...
 const visible=rows.filter(r=>{
  const q=filter.trim().toLowerCase();return !q||[r.business_name,r.owner_name,r.city_name,r.plan_name,r.plan_code].filter(Boolean).join(' ').toLowerCase().includes(q)
 })
 return <section className="admin-card">
  ...
  {loading?<div className="empty">Carregando empresas...</div>:!visible.length?<div className="empty"><h3>Nenhuma empresa encontrada.</h3></div>:<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Empresa</th><th>Proprietário</th><th>Cidade</th><th>Plano atual</th><th>Validade opcional</th><th>Novo plano</th></tr></thead><tbody>{visible.map(row=><tr key={row.business_id}>
   ...
   <td><span className="status-pill">{row.plan_name||row.plan_code}</span>{row.subscription_ends_at&&<small className="muted">até {new Date(row.subscription_ends_at).toLocaleDateString('pt-BR')}</small>}</td>
   <td><input type="date" value={ends[row.business_id]||''} onChange={e=>setEnds(x=>({...x,[row.business_id]:e.target.value}))} title="Opcional: ao chegar nesta data a empresa volta ao plano gratuito"/></td>
