...
async function handleCityToggle(event){
 ...
 if(!slug){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();showError('Não foi possível identificar a cidade para atualizar.');return}
 event.preventDefault();event.stopPropagation();event.stopImmediatePropagation()
 const original=button.textContent
 button.disabled=true;button.textContent='Salvando…'
 try{
  const {error}=await db.from('cities').update({active:action==='Ativar',updated_at:new Date().toISOString()}).eq('slug',slug)
  if(error)throw error
  window.location.reload()
 }catch(error){button.disabled=false;button.textContent=original;showError(error?.message||'Não foi possível atualizar o status da cidade.')}
}

if(location.pathname.startsWith('/admin'))document.addEventListener('click',handleCityToggle,true)
