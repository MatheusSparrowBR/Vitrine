...
export default function AdminHomePage(){
 ...
 return <AdminShell active="dashboard" title="Visão geral" description="Uma central única para operar catálogo, publicidade, agenda e modelo comercial da VitrineLocal." email={session?.user?.email}>
  ...
  <section className="admin-v2-grid-2 admin-home-main-grid">
   <div className="admin-v2-card admin-v2-section admin-home-actions-card"><div className="admin-v2-section-head"><div><span className="admin-v2-kicker">ATALHOS</span><h2>Ações rápidas</h2><p>Os fluxos mais usados, organizados para reduzir cliques.</p></div></div><div className="admin-v2-quick-grid admin-home-quick-grid">{actions.map(([key,label,text,href])=><a className="admin-v2-quick admin-home-quick" href={href} key={key}><span>{key==='promotions'?'％':key==='events'?'◫':key==='banners'?'▰':'▦'} · {label}</span><strong>Abrir gestão</strong><small>{text}</small><b className="admin-home-quick-arrow">→</b></a>)}</div></div>
