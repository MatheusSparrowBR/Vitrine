...
export default function BusinessReviews({businessId}){
 ...
 const load=async()=>{
  ...
  setD({
   avg:Number(summary.avg_rating||0),
   ...
   distribution:{
    ...
    2:Number(summary.two_stars||0),
    3:Number(summary.three_stars||0),
    4:Number(summary.four_stars||0),
    5:Number(summary.five_stars||0)
   },
   loading:false,
   error:results[1]?.error?.message||results[0]?.error?.message||''
  })
 }
 ...
 return <section className="vl-reviews" aria-label="Avaliações dos clientes">
  <div className="vl-reviews-head">
   <div>
    ...
    <p>Veja a experiência de quem já conheceu esta empresa.</p>
   </div>
   <div className="vl-reviews-score">
    <strong>{d.avg.toFixed(1)}</strong>
    <span aria-label={`${d.avg.toFixed(1)} de 5 estrelas`}>★★★★★</span>
    <small>{scoreLabel}</small>
   </div>
  </div>

  <div className="vl-reviews-summary">
   <div className="vl-review-breakdown">
    {bars.map(x=><div className="vl-review-bar" key={x.n}>
     <span>{x.n} <b>★</b></span><i><b style={{width:`${x.pct}%`}}/></i><em>{x.count}</em>
    </div>)}
   </div>
   <div className="vl-review-meta">
    <div><strong>{d.count}</strong><span>Avaliações</span></div>
    <div><strong>{d.avg?d.avg.toFixed(1):'—'}</strong><span>Nota média</span></div>
   </div>
  </div>
  ...
  {d.loading?<div className="vl-review-login vl-review-auth-loading"><div><strong>Verificando sua conta…</strong><span>Estamos preparando a área de avaliação.</span></div></div>:d.session?<div className="vl-review-form">
   ...
   <div className="vl-review-stars-input" role="radiogroup" aria-label="Escolha uma nota de 1 a 5">
    <small>{r}/5</small>
    {[1,2,3,4,5].map(n=><button key={n} type="button" className={n<=r?'active':''} onClick={()=>setR(n)} aria-label={`${n} estrelas`} aria-pressed={n<=r}>★</button>)}
   </div>
