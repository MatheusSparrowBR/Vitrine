...
function when(v){return v?new Date(v).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'}):'Sem período'}
...
export default function AdminPromotionsPage({supabase}){
 ...
 const [editor,setEditor]=useState(null),[form,setForm]=useState(empty),[imageFile,setImageFile]=useState(null),[preview,setPreview]=useState(''),[saving,setSaving]=useState(false),[message,setMessage]=useState({text:'',error:false}),[statusFilter,setStatusFilter]=useState('all'),[search,setSearch]=useState('')
 ...
 async function save(e){
  ...
  const starts=localInputToISO(form.starts_at),ends=localInputToISO(form.ends_at)
 ...
 return <div className="admin-promotions-page">
  ...
  <section className="admin-promo-toolbar-card">
   <div><span className="admin-promo-eyebrow">CATÁLOGO COMERCIAL</span><h2>Promoções</h2><p>Crie e agende ofertas com horário local preciso e imagem personalizada.</p></div>
