...
function App(){
 ...
 useEffect(()=>{
  ...
  const find=async()=>{
   ...
   if(!session||!live){if(live)setBusinessId(null);return}
   const {data}=await db.from('businesses').select('id').eq('owner_id',session.user.id).order('created_at',{ascending:false}).limit(20)
   if(!live)return
   const ids=(data||[]).map(x=>x.id)
   const picker=[...document.querySelectorAll('.business-picker button')]
   const active=picker.findIndex(x=>x.classList.contains('active'))
   setBusinessId(ids[Math.max(0,active)]||ids[0]||null)
  }
  find()
  const {data:auth}=db.auth.onAuthStateChange(()=>find())
  const onBusinessClick=()=>setTimeout(find,0)
  document.addEventListener('click',e=>{if(e.target.closest?.('.business-picker button'))onBusinessClick()})
  return()=>{live=false;auth.subscription.unsubscribe();document.removeEventListener('click',e=>{if(e.target.closest?.('.business-picker button'))onBusinessClick()})}
 },[])
