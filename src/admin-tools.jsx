...
export default function AdminTools({ supabase, session, onClose, onToast }) {
  ...
  async function loadAll() {
    setLoading(true)
    const [c1,c2,p,b,ads] = await Promise.all([
  ...
  async function saveCity(event) {
    ...
    try {
      ...
      onToast?.(editingCity ? 'Cidade atualizada.' : 'Cidade criada.')
      resetCity(); await loadAll()
  ...
  async function saveCategory(event) {
    ...
    try {
      ...
      if(result.error) throw result.error
      onToast?.(editingCategory ? 'Categoria atualizada.' : 'Categoria criada.')
  ...
  async function createPromotion(event) {
    ...
    const { error } = await supabase.from('promotions').insert({
      business_id:promotionForm.business_id,
      title:promotionForm.title.trim(),
      description:promotionForm.description.trim() || null,
      price:promotionForm.price === '' ? null : Number(promotionForm.price),
      original_price:promotionForm.original_price === '' ? null : Number(promotionForm.original_price),
      starts_at:promotionForm.starts_at ? new Date(promotionForm.starts_at).toISOString() : null,
  ...
  async function createBanner(event) {
    event.preventDefault(); setBusy('banner')
    let path = ''
    try {
      ...
      const { error } = await supabase.from('advertisements').insert({
        business_id:bannerForm.business_id,
        city_id:bannerForm.city_id,
        title:bannerForm.title.trim(),
        description:bannerForm.description.trim() || null,
        image_url:url,
        image_path:path,
        target_url:bannerForm.target_url.trim() || null,
      ...
      onToast?.('Banner criado em revisão.')
      setBannerForm({business_id:'',city_id:'',title:'',description:'',target_url:'',priority:0,starts_at:'',ends_at:'',image:null})
      await loadAll()
    } catch(e) {
      if(path) await supabase.storage.from(BUCKET).remove([path]).catch(()=>{})
      onToast?.(e.message || 'Não foi possível criar o banner.',true)
    } finally { setBusy('') }
  }

  async function toggleBanner(row) {
    setBusy(row.id)
    const { error } = await supabase.from('advertisements').update({active:!row.active}).eq('id',row.id)
    if(error) onToast?.(error.message,true); else { onToast?.(row.active ? 'Banner pausado.' : 'Banner publicado.'); await loadAll() }
    setBusy('')
  }
  ...
  async function deleteBanner(row) {
    ...
    if(error) onToast?.(error.message,true); else { if(row.image_path) await supabase.storage.from(BUCKET).remove([row.image_path]).catch(()=>{}); onToast?.('Banner excluído.'); await loadAll() }
    setBusy('')
  }
  ...
  return <div className="vl-admin-tools-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose?.()}>
    <div className="vl-admin-tools-modal">
      <div className="vl-admin-tools-head"><div><span>CONTROLES DA PLATAFORMA</span><h2>Gestão administrativa</h2><p>Categorias, cidades, promoções e banners Premium centralizados em um único lugar.</p></div><button onClick={onClose}>×</button></div>
      <div className="vl-admin-tools-tabs">
        {[['cities','Cidades'],['categories','Categorias'],['promotions','Promoções'],['banners','Banners Premium']].map(([id,label])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)}>{label}{id==='promotions'&&pendingPromotions.length>0?<b>{pendingPromotions.length}</b>:null}</button>)}
      </div>
