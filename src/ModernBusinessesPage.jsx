...
export default function ModernBusinessesPage({citySlug='laguna'}){
 ...
 return <div className="mbl-shell"><main className="mbl-page">
  ...
  <section className="mbl-toolbar" aria-label="Filtros do catálogo">
   <div className="mbl-toolbar-row"><div className="mbl-search-large"><Icon name="search" size={18}/><input value={q} onChange={e=>setSearch(e.target.value)} placeholder="Buscar empresa, serviço ou bairro" aria-label="Buscar no catálogo"/></div>{(q||cat||need||Object.values(quickFilters).some(Boolean)||sort!=='relevancia')&&<button className="mbl-clear" type="button" onClick={clearFilters}>Limpar</button>}</div>
   ...
   <div className="mbl-sort-row"><span className="mbl-sort-label">Ordenar por</span><select className="mbl-sort-select" value={sort} onChange={e=>setSortValue(e.target.value)} aria-label="Ordenar resultados"><option value="relevancia">Relevância</option><option value="avaliacao">Melhor avaliadas</option><option value="recentes">Mais recentes</option></select></div><div className="mbl-quick-filter-head"><strong>Filtros rápidos</strong><button type="button" onClick={()=>setShowMoreFilters(v=>!v)}>{showMoreFilters?'Ocultar':'Mais filtros'} {Object.values(quickFilters).filter(Boolean).length>0?'· '+Object.values(quickFilters).filter(Boolean).length:''}</button></div>{showMoreFilters&&<div className="mbl-quick-filters">{[['open','Aberto agora'],['delivery','Delivery'],['pickup','Retirada'],['dine','Consumo no local'],['verified','Verificada'],['searchFeatured','Busca em destaque']].map(([key,label])=><button key={key} type="button" className={quickFilters[key]?'active':''} onClick={()=>updateQuickFilter(key)}>{quickFilters[key]?'✓ ':''}{label}</button>)}</div>}
