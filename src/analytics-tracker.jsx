...
function pwaInstallId(){
 try{
  ...
  if(!id){id=crypto.randomUUID();localStorage.setItem('vl_pwa_install_id',id)}
  return id
 }catch{return null}
}
...
async function recordPwaInstall(method){
 try{
...
export default function AnalyticsTracker(){useEffect(()=>{if(location.pathname.startsWith('/admin')||location.pathname==='/login'||location.pathname==='/atualizar-senha')return;const parts=location.pathname.split('/').filter(Boolean);record('page_view');if(parts[1]==='empresa')record('profile_view');const seen=new Set();const trackBanner=()=>document.querySelectorAll('[data-ad-id],.vl-premium-banner,.ad-card').forEach(el=>{const id=el.getAttribute('data-ad-id')||'';const title=el.querySelector('.vl-premium-copy strong')?.textContent?.trim()||el.querySelector('.ad-content h2')?.textContent?.trim()||'Banner Premium';const href=el.getAttribute('href')||el.querySelector('a')?.getAttribute('href')||'';const key=id||`${title}|${href}`;if(seen.has(key))return;seen.add(key);record('banner_impression',{advertisement_id:id||null,title,href})});trackBanner();
  const onPwaInstalled=()=>{void recordPwaInstall('appinstalled')}
  window.addEventListener('appinstalled',onPwaInstalled)
  if(isStandalonePwa())void recordPwaInstall('standalone_launch')
  const observer=new MutationObserver(trackBanner);observer.observe(document.getElementById('root')||document.body,{childList:true,subtree:true});const click=e=>{const a=e.target.closest?.('a,button');if(!a)return;const href=a.href||'';const text=(a.innerText||'').trim();const track=a.dataset.track||'';const adId=a.getAttribute('data-ad-id')||a.closest?.('[data-ad-id]')?.getAttribute('data-ad-id')||'';const adCard=a.closest?.('.ad-card');if(track==='share')record('share_click');else if(track==='save')record('save_click');else if(track==='instagram')record('instagram_click');else if(track==='whatsapp')record('whatsapp_click');else if(a.matches?.('[data-gallery-open]'))record('gallery_open');else if(adId||a.matches?.('.vl-premium-banner')||adCard)record('banner_click',{advertisement_id:adId||null,title:a.querySelector('.vl-premium-copy strong')?.textContent?.trim()||adCard?.querySelector('.ad-content h2')?.textContent?.trim()||'Banner Premium',href});else if(a.matches?.('[href="/planos"]')||/\/planos(?:\?|$)/i.test(href))record('plan_view');else if(/conta\?new=business/i.test(href)||/conta\/nova(?:\?|$)/i.test(href))record('business_signup_start');else if(href.includes('wa.me')||/whatsapp/i.test(text))record('whatsapp_click');else if(/instagram\.com/i.test(href)||/instagram/i.test(text))record('instagram_click');else if(a.matches?.('[href*="/empresa/"]'))record('business_click');else if(a.matches?.('[href*="/promocoes"]'))record('promotion_click');else if(a.matches?.('[href*="/eventos"]'))record('event_click');else if(a.matches?.('[href*="categoria="]'))record('category_click',{category:new URL(href,location.origin).searchParams.get('categoria')});else if(/^https?:/i.test(href)&&!href.includes(location.host))record('website_click')};document.addEventListener('click',click);return()=>{observer.disconnect();document.removeEventListener('click',click);window.removeEventListener('appinstalled',onPwaInstalled)}},[]);return null}
