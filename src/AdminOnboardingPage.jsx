...
export default function AdminOnboardingPage({mode='partner'}){
 ...
 const canSubmit=useMemo(()=>{if(mode==='business')return Boolean(business.owner_id&&business.name.trim()&&business.city_id);if(!user.full_name.trim()||!user.email.trim())return false;if(mode==='partner')return !business.name.trim()||Boolean(business.city_id);return true},[mode,user,business])
