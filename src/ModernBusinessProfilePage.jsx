...
function Gallery({business,photos}){
 const images=photos.filter(p=>p.media_type!=='video'&&p.url)
 const media=photos.filter(p=>p.url)
 ...
 const extra=Math.max(0,media.length-2)
 return <div className="mbp-gallery">
  <button className="mbp-gallery-main" type="button" aria-label="Abrir galeria de fotos">
  ...
  <div className="mbp-gallery-side">
   {thumbs.map((p,i)=><button className="mbp-gallery-thumb" type="button" key={p.id||i} aria-label={`Abrir foto ${i+1}`}><img src={p.url} alt="" loading="lazy"/></button>)}
   <button className="mbp-gallery-more" type="button">{extra>0&&<strong>+{extra}</strong>}<span>Ver todas as fotos</span></button>
  </div>
 </div>
}
...
export default function ModernBusinessProfilePage({citySlug='laguna',businessSlug=''}){
 ...
 const openNow=hours.some(h=>!h.closed)
   ...
   <div className="mbp-layout">
    <section className="mbp-main-card">
     ...
     <div className="mbp-company-head">
      ...
      <div className="mbp-company-copy"><div className="mbp-kicker">EMPRESA LOCAL</div><h1>{business.name}</h1><div className="mbp-badges">{rating!=null&&reviewCount>0?<span className="mbp-rating">★ {Number(rating).toFixed(1)} <small>({reviewCount})</small></span>:<span className="mbp-rating">★ Sem avaliações</span>}{business.verified&&<span className="mbp-verified">✓ Verificada</span>}<span className="mbp-category">▤ {business.categories?.name||'Empresa'}</span></div><p>{business.short_description||business.description||'Conheça esta empresa local.'}</p></div>
     </div>
     <div className="mbp-contact-grid"><ContactCard icon="⌖" title="Localização" value={profileLocation||'Laguna - SC'} link={business.address?`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(business.address)}`:undefined}/><ContactCard icon="◷" title={openNow?'Aberto agora':'Horário'} value={hours[0]?.text||'Consulte os horários'}/>{wa&&<ContactCard icon="☎" title="WhatsApp" value={business.phone||business.whatsapp} link={`https://wa.me/${phoneDigits(wa)}`}/>} {business.instagram_url&&<ContactCard icon="◎" title="Instagram" value={business.instagram_url.replace(/^https?:\/\/(www\.)?instagram\.com\//,'@').replace(/\/$/,'')} link={business.instagram_url}/>}</div>
     ...
     {items.length>0&&<section className="mbp-section"><div className="mbp-section-title"><span>PRODUTOS E SERVIÇOS</span><h2>O que a empresa oferece</h2></div><div className="mbp-items">{items.slice(0,6).map(i=><article key={i.id}>{i.image_url&&<img src={i.image_url} alt="" loading="lazy"/>}<div><h3>{i.name}</h3>{i.description&&<p>{i.description}</p>}{i.price!=null&&<strong>{fmt(i.price)}</strong>}</div></article>)}</div></section>}
    </section>
    <aside className="mbp-sidebar"><div className="mbp-tabs" role="tablist">{[['about','Sobre'],['hours','Horário'],['reviews','Avaliações'],['location','Localização']].map(([id,label])=><button key={id} role="tab" aria-selected={tab===id} className={tab===id?'active':''} onClick={()=>setTab(id)}>{label}</button>)}</div>
     ...
     {promotions.length>0&&<section className="mbp-side-card mbp-promos"><div className="mbp-side-icon">%</div><span className="mbp-side-kicker">OFERTAS</span><h2>Promoções</h2>{promotions.slice(0,3).map(p=><div className="mbp-promo" key={p.id}><strong>{p.title}</strong>{p.price!=null&&<span>{fmt(p.price)}</span>}</div>)}</section>}
    </aside>
   </div>
  </main>
 </div>
