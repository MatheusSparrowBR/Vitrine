...
const expired = p => Boolean(p.ends_at && new Date(p.ends_at) < new Date())
...
function render(){
  ...
  const counts={pending_review:state.rows.filter(p=>p.status==='pending_review').length,published:state.rows.filter(p=>p.status==='published').length,rejected:state.rows.filter(p=>p.status==='rejected').length,archived:state.rows.filter(p=>p.status==='archived').length,expired:state.rows.filter(expired).length}
  const rows=visible()
