...
async function loadPublicBanners() {
  ...
  if (!cityId) return []
  const { data } = await supabase.from('advertisements').select('id,title,description,image_url,target_url,priority').eq('city_id', cityId).eq('placement', 'home_banner').eq('active', true).order('priority', { ascending: false }).order('created_at', { ascending: false }).limit(5)
  return data || []
}
...
async function boot() {
  ...
  const observer = new MutationObserver(async () => {
    ...
    if (hero && !state.publicBooted) {
      ...
      if (state.ads.length > 1 && !state.rotationStarted) { state.rotationStarted = true; window.setInterval(() => { if (!document.querySelector('.hero')) return; state.index = (state.index + 1) % state.ads.length; renderPublicBanner() }, 6000) }
    }
