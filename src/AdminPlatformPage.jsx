...
export default function AdminPlatformPage({supabase}){
 ...
 return <AdminShell active={activeKey} title={title} description={description} email={session.user.email}>
  {tab==='events'?<section className="admin-v2-card admin-v2-section admin-v2-event-launch"><div className="admin-v2-section-head"><div><span className="admin-v2-kicker">AGENDA</span><h2>Gestão de eventos</h2><p>Cadastre, edite, destaque, publique e exclua eventos sem sair da central administrativa.</p></div></div><div className="admin-v2-event-card"><div><strong>Agenda da cidade</strong><span>O editor completo de eventos abre em uma janela de trabalho otimizada para desktop e mobile.</span></div><AdminEventsPanel supabase={supabase}/></div></section>:tab==='promotions'?<AdminPromotionsPage supabase={supabase}/>:tab==='assignments'?<AdminPlanAssignments supabase={supabase}/>:<AdminTools supabase={supabase} defaultTab={tab}/>} 
 </AdminShell>
}
