...
export default function AdminPlatformPage({supabase}){
 ...
 return <div className="app" style={{minHeight:'100vh',background:'#eef3f8'}}><AdminTools supabase={supabase} session={session} onClose={close} onToast={(text,error=false)=>{setToast({text,error});window.setTimeout(()=>setToast({text:'',error:false}),3200)}} /><AdminEventsPanel supabase={supabase}/>{toast.text&&<div style={{position:'fixed',top:18,left:'50%',transform:'translateX(-50%)',zIndex:12000,padding:'11px 16px',borderRadius:11,background:toast.error?'#fff0f0':'#10253e',color:toast.error?'#a12e2e':'#fff',boxShadow:'0 12px 28px rgba(0,0,0,.18)',fontWeight:800,fontSize:12}}>{toast.text}</div>}</div>
}
