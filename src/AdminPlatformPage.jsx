...
export default function AdminPlatformPage({supabase}){
 ...
 const titles={promotions:['Promoções','Crie, agende e gerencie ofertas com horário local, imagem e controle de publicação.'],cities:['Cidades','Gerencie as cidades atendidas pela plataforma.'],needs:['Necessidades','Configure as intenções de descoberta e conecte categorias ao caminho de exploração do usuário.'],categories:['Categorias','Organize as categorias das empresas e associe cada uma às necessidades relevantes.'],plans:['Planos','Edite preços, descrições e benefícios diretamente pelo painel.'],assignments:['Planos por empresa','Conceda Pro ou Premium manualmente, defina validade e acompanhe o plano efetivo de cada empresa.'],events:['Eventos','Cadastre e modere a agenda pública da cidade.']}
 ...
 return <AdminShell active={activeKey} title={title} description={description} email={session.user.email}>
  {tab==='events'?<AdminEventsMain supabase={supabase}/>:tab==='promotions'?<AdminPromotionsPage supabase={supabase}/>:tab==='assignments'?<AdminPlanAssignments supabase={supabase}/>:tab==='plans'?<AdminPlansPage supabase={supabase}/>:<AdminTools supabase={supabase} defaultTab={tab}/>} 
 </AdminShell>
}
