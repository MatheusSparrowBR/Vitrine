...
export default function AdminPlatformPage({supabase}){
 ...
 const titles={promotions:['Promoções','Crie, agende e gerencie ofertas com horário local, imagem e controle de publicação.'],cities:['Cidades','Gerencie as cidades atendidas pela plataforma.'],categories:['Categorias','Organize a descoberta de empresas e serviços.'],plans:['Planos','Ajuste a estrutura comercial e os benefícios disponíveis.'],events:['Eventos','Cadastre e modere a agenda pública da cidade.']}
 ...
 return <AdminShell active={tab==='events'?'events':tab} title={title} description={description} email={session.user.email}>
  {tab==='events'?<section className="admin-v2-card admin-v2-section admin-v2-event-launch"><div className="admin-v2-section-head"><div><span className="admin-v2-kicker">AGENDA</span><h2>Gestão de eventos</h2><p>Cadastre, edite, destaque, publique e exclua eventos sem sair da central administrativa.</p></div></div><div className="admin-v2-event-card"><div><strong>Agenda da cidade</strong><span>O editor completo de eventos abre em uma janela de trabalho otimizada para desktop e mobile.</span></div><AdminEventsPanel supabase={supabase}/></div></section>:tab==='promotions'?<AdminPromotionsPage supabase={supabase}/>:<AdminTools supabase={supabase} defaultTab={tab}/>} 
 </AdminShell>
}
