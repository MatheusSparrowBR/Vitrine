...
const state = { open: false, editingId: null, rows: [], booted: false }
...
function render() {
  ...
  nameInput.addEventListener('input', () => { if (!editing && !slugInput.value) slugInput.value = slugify(nameInput.value) })
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      status.textContent = error?.message || 'Não foi possível salvar a cidade.'
...
async function openModal() {
  ...
  root.id = 'vlcity-admin-root'
  document.body.appendChild(root)
  try { await loadCities(); render(); setTimeout(() => root.querySelector('[name="name"]')?.focus(), 0) }
  catch (error) { root.innerHTML = `<div class="vlcity-backdrop"><div class="vlcity-modal"><button class="vlcity-close" id="vlcity-close" type="button">×</button><h2>Erro ao carregar cidades</h2><p>${esc(error?.message || 'Erro desconhecido.')}</p></div></div>`; root.querySelector('#vlcity-close').onclick = closeModal }
}

function normalizeCityButton(button) {
  if (!button) return false
  if ((button.textContent || '').replace(/\s+/g, ' ').trim() !== 'Cidades') return false
  if (button.dataset.vlCitiesAdmin === 'true') return true
...
function boot() {
  ...
  const capture = async (event) => {
    ...
    if (!button) return
    if ((button.textContent || '').replace(/\s+/g, ' ').trim() !== 'Cidades') return
    event.preventDefault()
    event.stopPropagation()
    event.stopImmediatePropagation()
    await openModal()
  ...
  const observer = new MutationObserver(() => {
    ...
    if (!nav) return
    const button = [...nav.querySelectorAll('button')].find((item) => (item.textContent || '').replace(/\s+/g, ' ').trim() === 'Cidades')
    normalizeCityButton(button)
  })
