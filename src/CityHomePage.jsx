...
export default function CityHomePage({citySlug='laguna'}){
 ...
 useEffect(()=>{if(!db||!city?.id||city.id==='fallback')return;let alive=true;setLoading(true);setEventError('');setPromoError('')
  ...
  const loadBusinesses=async()=>{try{const r=await withTimeout(db.from('public_business_directory').select('id,name,slug,short_description,cover_url,address,featured,verified,category_name,created_at').eq('city_id',city.id).order('featured',{ascending:false}).order('created_at',{ascending:false}).limit(8));if(r.error)throw r.error;const rows=r.data||[];let reviewMap={};const ids=rows.map(row=>row.id).filter(Boolean);if(ids.length){const results=await Promise.all(ids.map(id=>withTimeout(db.rpc('get_public_business_reviews',{p_business_id:id,p_limit:100}))));results.forEach((reviews,index)=>{if(reviews.error)return;(reviews.data||[]).forEach(row=>{const businessId=ids[index];const current=reviewMap[businessId]||{sum:0,count:0};current.sum+=Number(row.rating)||0;current.count+=1;reviewMap[businessId]=current})})}if(!alive)return;setBusinesses(rows.map(row=>({...row,categories:row.category_name?{name:row.category_name}:null})));setRatings(reviewMap)}catch(_){if(alive){setBusinesses([]);setRatings({})}}}
 ...
 return <div className='lvp-page'>
  <main>
   ...
   <section id='explorar' className='lvp-wrap lvp-featured'>
    ...
    {featuredBusinesses.length?<div className={`lvp-business-grid ${featuredBusinesses.length===1?'lvp-business-grid-single':''}`}>{featuredBusinesses.slice(0,4).map(b=>{const stat=ratings[b.id]||{sum:0,count:0};const avg=stat.count?stat.sum/stat.count:0;return <a href={base+'/empresa/'+encodeURIComponent(b.slug)} className='lvp-business-card' key={b.id}>
      ...
      <div className='lvp-business-body'><span className='lvp-card-cat'>{b.categories?.name||'Empresa local'}</span><div className='lvp-card-rating' aria-label={stat.count?`${avg.toFixed(1)} de 5, ${stat.count} avaliações`:'Sem avaliações'}><span className='lvp-card-rating-stars' aria-hidden='true'>{stat.count?stars(avg):'☆'}</span>{stat.count?<><strong>{avg.toFixed(1)}</strong><small>· {stat.count} {stat.count===1?'avaliação':'avaliações'}</small></>:<small>Sem avaliações</small>}</div><h3>{b.name}</h3><p>{b.short_description||'Conheça este negócio local.'}</p>{b.address&&<small className='lvp-card-address'>📍 {b.address}</small>}<span className='lvp-card-link'>Ver empresa →</span></div>
