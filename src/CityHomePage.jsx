...
export default function CityHomePage({citySlug='laguna'}){
 ...
 useEffect(()=>{if(!db||!city?.id||city.id==='fallback')return;let alive=true;setLoading(true);setEventError('');setPromoError('')
  ...
  const loadBusinesses=async()=>{try{const r=await withTimeout(db.from('public_business_directory').select('id,name,slug,short_description,cover_url,address,featured,verified,category_name,created_at').eq('city_id',city.id).order('featured',{ascending:false}).order('created_at',{ascending:false}).limit(8));if(r.error)throw r.error;const rows=r.data||[];const reviewMap=await loadPublicBusinessReviewSummaries(db,rows.map(row=>row.id));if(!alive)return;setBusinesses(rows.map(row=>({...row,categories:row.category_name?{name:row.category_name}:null})));setRatings(reviewMap)}catch(_){if(alive){setBusinesses([]);setRatings({})}}}
