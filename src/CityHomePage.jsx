...
export default function CityHomePage({citySlug='laguna'}){
 ...
 useEffect(()=>{if(!db||!city?.id)return;let alive=true;setLoading(true);setPromoError('')
  const loadCategories=async()=>{try{const r=await db.from('categories').select('id,name,slug,icon').eq('active',true).order('sort_order').order('name');if(!alive)return;setCategories(r.data?.length?r.data:FALLBACK_CATS.map(([name,icon],i)=>({id:i,name,icon,slug:slugify(name)})))}catch(_){if(alive)setCategories(FALLBACK_CATS.map(([name,icon],i)=>({id:i,name,icon,slug:slugify(name)})))}}
  const loadBusinesses=async()=>{try{const r=await db.from('public_business_directory').select('id,name,slug,short_description,cover_url,address,featured,verified,category_name,created_at').eq('city_id',city.id).order('featured',{ascending:false}).order('created_at',{ascending:false}).limit(8);if(!alive)return;setBusinesses((r.data||[]).map(row=>({...row,categories:row.category_name?{name:row.category_name}:null})))}catch(_){if(alive)setBusinesses([])}}
  const loadPromotions=async()=>{try{const r=await getActiveCityPromotions(db,city.id,{limit:100});if(!alive)return;if(r.error){setPromoError(r.error.message||'Não foi possível carregar as promoções.');setPromotions([])}else setPromotions(r.data||[])}catch(error){if(alive){setPromotions([]);setPromoError(error?.message||'Não foi possível carregar as promoções.')}}}
  const loadEvents=async()=>{try{const r=await db.from('events').select('id,title,description,image_url,event_date,start_time,location,featured').eq('city_id',city.id).eq('active',true).gte('event_date',projectTodayISO()).order('featured',{ascending:false}).order('event_date').order('start_time').limit(4);if(!alive)return;if(r.error){setEvents([])}else setEvents(r.data||[])}catch(_){if(alive)setEvents([])}finally{if(alive)setLoading(false)}}
  const loadBanners=async()=>{try{const r=await db.from('advertisements').select('id,title,description,image_url,target_url,priority,starts_at,ends_at').eq('city_id',city.id).eq('placement','home_banner').eq('active',true).order('priority',{ascending:false}).order('created_at',{ascending:false}).limit(20);if(!alive)return;const now=Date.now();const eligible=(r.data||[]).filter(x=>(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now));setBanners(eligible);setBannerIndex(0)}catch(_){if(alive){setBanners([]);setBannerIndex(0)}}}
