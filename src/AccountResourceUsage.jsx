...
const RESOURCE_COPY={photos:{label:'Mídias',icon:'image',description:'Capacidade de mídias atualmente armazenadas.'},items:{label:'Produtos e serviços',icon:'grid',description:'Capacidade de produtos ou serviços atualmente cadastrados.'}}
...
export default function AccountResourceUsage({businessId,resource,used:legacyUsed=0,onState,onAction,actionLabel='Adicionar',showAction=true}){
 ...
 const isCapacityResource=resource==='photos'||resource==='items'
 ...
 return <section className={`vl-resource-usage ${reached?'reached':''}`}>
  <div className="vl-resource-usage-main"><div className="vl-resource-ring" style={{'--usage':`${percent}%`}}><div><strong>{usedCount}</strong><span>{unlimited?'Ilimitado':`de ${safeLimit}`}</span></div></div><div className="vl-resource-copy"><span className="account-eyebrow">SEU PLANO · {planName.toUpperCase()}</span><h2>{unlimited?'Uso ilimitado para testes':reached&&safeLimit>0?isCapacityResource?`Limite de ${copy.label.toLowerCase()} atingido`:`Limite mensal de ${copy.label.toLowerCase()} atingido`:isCapacityResource?`Capacidade de ${copy.label.toLowerCase()}`:`Uso de ${copy.label.toLowerCase()} no ciclo`}</h2><p>{unlimited?`Esta empresa está vinculada à conta administradora e não possui limite de ${copy.label.toLowerCase()} para testes.`:safeLimit>0?isCapacityResource?`${usedCount} de ${safeLimit} ${copy.label.toLowerCase()} estão armazenados atualmente. Excluir uma mídia ou produto libera capacidade.`:`${usedCount} de ${safeLimit} ${copy.label.toLowerCase()} já foram utilizados neste ciclo.`:'Este recurso não está disponível no plano atual.'}</p><div className="vl-resource-progress"><i style={{width:`${percent}%`}}/></div>{usage?.cycleEnd&&!unlimited&&<small>{isCapacityResource?'A capacidade é liberada quando você remove uma mídia ou produto.':`O limite mensal será renovado em ${formatDate(usage.cycleEnd)}.`}</small>}</div></div>
  <div className="vl-resource-side"><strong>{unlimited?'∞':`${percent}%`}</strong><span>{unlimited?'limite para testes':isCapacityResource?'capacidade utilizada':'utilizado no ciclo'}</span>{safeLimit>0&&!unlimited&&<small>{remaining} restante{remaining===1?'':'s'}</small>}{reached&&safeLimit>0&&<b>Limite atingido</b>}</div>
  {reached?<div className="vl-resource-upgrade"><div><span><Icon name="briefcase" size={15}/></span><div><strong>Quer continuar adicionando?</strong><p>Faça upgrade para aumentar o limite disponível para sua empresa.</p></div></div><a href={`/planos?business_id=${encodeURIComponent(businessId)}`}>Ver planos e aumentar limite →</a></div>:showAction?<button type="button" className="vl-resource-create" onClick={onAction}><Icon name={copy.icon} size={14}/> {actionLabel}</button>:null}
 </section>
}
