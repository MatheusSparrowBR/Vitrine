...
export default function AccountResourceUsage({businessId,resource,used:legacyUsed=0,onState,onAction,actionLabel='Adicionar'}){
 ...
 const usedCount=Number(usage?.used)||0,safeLimit=Number(usage?.limit)||0,unlimited=safeLimit<0,percent=usagePercent(usedCount,safeLimit),reached=safeLimit===0||(safeLimit>0&&usedCount>=safeLimit),remaining=safeLimit>0?Math.max(0,safeLimit-usedCount):null,planName=PLAN_NAMES[usage?.planCode]||'Plano atual'
 ...
 return <section className={`vl-resource-usage ${reached?'reached':''}`}>
  ...
  <div className="vl-resource-side"><strong>{unlimited?'∞':`${percent}%`}</strong><span>{unlimited?'limite para testes':'utilizado no ciclo'}</span>{safeLimit>0&&<small>{remaining} restante{remaining===1?'':'s'}</small>}{reached&&safeLimit>0&&<b>Limite atingido</b>}</div>
