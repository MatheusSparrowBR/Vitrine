...
export default function AccountResourceUsage({businessId,resource,used:legacyUsed=0,onState,onAction,actionLabel='Adicionar',showAction=true}){
 ...
 return <section className={`vl-resource-usage ${reached?'reached':''}`}>
  ...
  {reached?<div className="vl-resource-upgrade"><div><span>🔒</span><div><strong>Quer continuar adicionando?</strong><p>Faça upgrade para aumentar o limite. O consumo já realizado neste ciclo não é apagado.</p></div></div><a href={`/planos?business_id=${encodeURIComponent(businessId)}`}>Ver planos e aumentar limite →</a></div>:showAction?<button type="button" className="vl-resource-create" onClick={onAction}>{copy.icon} {actionLabel}</button>:null}
 </section>
}
