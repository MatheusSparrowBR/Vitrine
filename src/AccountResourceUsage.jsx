...
const RESOURCE_COPY={photos:{label:'Mídias',icon:'image',description:'Unidades de mídia consumidas neste ciclo.'},items:{label:'Produtos e serviços',icon:'grid',description:'Novos produtos ou serviços consumidos neste ciclo.'}}
...
export default function AccountResourceUsage({businessId,resource,used:legacyUsed=0,onState,onAction,actionLabel='Adicionar',showAction=true}){
 ...
 return <section className={`vl-resource-usage ${reached?'reached':''}`}>
  ...
  {reached?<div className="vl-resource-upgrade"><div><span><Icon name="briefcase" size={15}/></span><div><strong>Quer continuar adicionando?</strong><p>Faça upgrade para aumentar o limite. O consumo já realizado neste ciclo não é apagado.</p></div></div><a href={`/planos?business_id=${encodeURIComponent(businessId)}`}>Ver planos e aumentar limite →</a></div>:showAction?<button type="button" className="vl-resource-create" onClick={onAction}><Icon name={copy.icon} size={14}/> {actionLabel}</button>:null}
 </section>
}
