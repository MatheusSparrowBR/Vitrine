...
export default function AdminDashboard({ supabase, session, onBack, onToast }) {
  ...
  const load = async () => {
    ...
    const [businessRes, submissionRes, postRes, cityRes] = await Promise.all([
      ...
      supabase.from('community_submissions').select('id,title,description,image_url,video_url,status,created_at,cities(name,state),profiles:user_id(full_name)').order('created_at', { ascending: false }).limit(100),
  ...
  async function moderateSubmission(item, approve) {
    ...
    if (approve) {
      ...
      if (postError) {
        setBusyId(''); onToast?.(postError.message, true); return
      }
      ...
      if (error) onToast?.(error.message, true)
      else onToast?.(`Conteúdo aprovado e publicado (${row?.id?.slice(0, 6) || 'post'}).`)
    } else {
  ...
  return <section className="admin-shell">
    <div className="container admin-layout">
      <aside className="admin-sidebar">
        <div className="admin-brand"><span className="brand-mark">V</span><div><strong>VitrineLocal</strong><small>Administração</small></div></div>
        <div className="admin-nav">{nav.map(([key, label, icon]) => <button className={tab === key ? 'admin-nav-item active' : 'admin-nav-item'} key={key} onClick={() => setTab(key)}><span>{icon}</span>{label}{key === 'businesses' && pendingBusinesses.length > 0 && <b>{pendingBusinesses.length}</b>}{key === 'community' && pendingSubmissions.length > 0 && <b>{pendingSubmissions.length}</b>}</button>)}</div>
        <button className="admin-back" onClick={onBack}>← Voltar ao site</button>
      </aside>

      <main className="admin-main">
        <div className="admin-head">
          <div><span className="section-kicker">Painel administrativo</span><h1>{nav.find((x) => x[0] === tab)?.[1]}</h1><p>Controle o conteúdo e o crescimento da VitrineLocal.</p></div>
          <div className="admin-user"><span>{session.user.email}</span><strong>ADMIN</strong></div>
        </div>

        {loading ? <div className="admin-loading">Carregando dados do painel…</div> : <>
          {tab === 'overview' && <>
            <div className="admin-stat-grid">
              <Stat label="Empresas" value={businesses.length} hint={`${pendingBusinesses.length} aguardando aprovação`} />
              <Stat label="Empresas ativas" value={activeBusinesses.length} hint="Visíveis no catálogo" />
              <Stat label="Conteúdo pendente" value={pendingSubmissions.length} hint="Envios da comunidade" />
              <Stat label="Posts publicados" value={publishedPosts.length} hint="Feed local" />
            </div>
            <div className="admin-two-col">
              <AdminPanel title="Fila de aprovação" action={() => setTab('businesses')} actionLabel="Ver empresas">
                {pendingBusinesses.slice(0, 5).map((b) => <ApprovalRow key={b.id} title={b.name} meta={`${b.categories?.name || 'Sem categoria'} · ${b.cities?.name || 'Sem cidade'}`} status="Pendente" onApprove={() => updateBusiness(b.id, { status: 'active', verified: true }, 'Empresa aprovada.')} onReject={() => updateBusiness(b.id, { status: 'rejected' }, 'Empresa rejeitada.')} busy={busyId === b.id} />)}
                {!pendingBusinesses.length && <AdminEmpty icon="✓" title="Tudo em dia" text="Nenhuma empresa aguardando aprovação." />}
              </AdminPanel>
              <AdminPanel title="Comunidade" action={() => setTab('community')} actionLabel="Moderar">
                {pendingSubmissions.slice(0, 5).map((s) => <ApprovalRow key={s.id} title={s.title || 'Sem título'} meta={`${s.cities?.name || 'Sem cidade'} · ${s.profiles?.full_name || 'Anônimo'}`} status="Pendente" />)}
                {!pendingSubmissions.length && <AdminEmpty icon="◉" title="Nenhum envio pendente" text="Os próximos vídeos e imagens da comunidade aparecerão aqui." />}
              </AdminPanel>
            </div>
          </>}

          {tab === 'businesses' && <>
            <div className="admin-toolbar"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar empresa, bairro ou cidade…" /><div className="admin-filters"><span>{pendingBusinesses.length} pendentes</span><span>{activeBusinesses.length} ativas</span></div></div>
