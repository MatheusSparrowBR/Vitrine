...
function RootRoute(){const path=normalizePath(location.pathname);if(path==='/login')return <AuthPage/>;if(path==='/usuario/login'||path==='/usuario/cadastro')return <UserAuthPage/>;if(path==='/usuario/perfil')return <UserProfilePage/>;if(path==='/planos')return <BillingPlansPage/>;if(path==='/conta')return <AccountRoute/>;if(path==='/conta/analytics')return <CommercialAnalyticsPage/>;if(path==='/conta/analytics-comercial')return <CommercialAnalyticsPage/>;if(path==='/conta/publicidade')return <MerchantAdvertisingSalesPage/>;if(path==='/conta/publicidade-legado')return <MerchantAdvertisingPage/>;if(path==='/conta/nova')return <BusinessRegistrationPage/>;if(path==='/atualizar-senha')return <PasswordUpdatePage/>;if(path==='/privacidade')return <PrivacyPage/>;if(path==='/termos')return <TermsPage/>;if(path==='/admin')return <AdminHomePage/>;if(path==='/admin/analytics')return <AdminAnalyticsPage/>;if(path==='/admin/empresas')return <AdminBusinessesPage/>;if(path==='/admin/usuarios')return <AdminUsersPage/>;if(path==='/admin/avaliacoes')return <AdminReviewsPage/>;if(path==='/admin/banners')return <AdminPremiumBannerPage supabase={supabase}/>;if(path==='/admin/publicidade')return <AdminAdvertisingSalesPage/>;if(path==='/admin/publicidade-legado')return <AdminAdvertisingPage/>;if(path==='/admin/gestao')return <AdminPlatformPage supabase={supabase}/>;const route=getCityRoute(path);if(!route)return <NotFoundPage/>;if(route.kind==='home')return <CityHomePage citySlug={route.citySlug}/>;if(route.kind==='events')return <EventRoute citySlug={route.citySlug}/>;if(route.kind==='businesses')return <ModernBusinessesPage citySlug={route.citySlug}/>;if(route.kind==='promotions')return <PromotionsPage citySlug={route.citySlug}/>;if(route.kind==='business')return <ModernBusinessProfilePage citySlug={route.citySlug} businessSlug={route.businessSlug}/>;return <NotFoundPage/>}
function App(){const isAdmin=location.pathname.startsWith('/admin');return <AppErrorBoundary><AccountModerationGuard><>{!isAdmin&&<SiteHeader/>}{!isAdmin&&<AnalyticsTracker/>}<RootRoute/></></AccountModerationGuard></AppErrorBoundary>}
