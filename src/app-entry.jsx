...
function EventRoute(){const [city,setCity]=React.useState(supabase?null:{id:'fallback',name:'Laguna',state:'SC',slug:location.pathname.split('/')[1]||'laguna',active:true}),[loading,setLoading]=React.useState(Boolean(supabase));const route=React.useMemo(()=>parseEventRoute(),[]);React.useEffect(()=>{let live=true;(async()=>{if(!supabase||!route?.citySlug){setLoading(false);return}const {data}=await supabase.from('cities').select('id,name,state,slug,country,active').eq('slug',route.citySlug).eq('active',true).maybeSingle();if(live){setCity(data||null);setLoading(false)}})();return()=>{live=false}},[route?.citySlug]);if(loading)return <div className="app"><div className="loader"/></div>;if(!city)return <div className="app"><main className="page section"><a className="link" href="/laguna">← Voltar</a><div className="empty"><h3>Cidade não encontrada.</h3></div></main></div>;return <div className="app"><EventsPage supabase={supabase} city={city} onBack={()=>location.href=`/${city.slug}`}/></div>}
function RootRoute(){const path=location.pathname;if(path==='/login')return <AuthPage/>;if(path==='/planos'||path==='/planos/')return <BillingPlansPage/>;if(path==='/privacidade'||path==='/privacidade/')return <PrivacyPage/>;if(path==='/termos'||path==='/termos/')return <TermsPage/>;if(path==='/admin/banners'||path==='/admin/banners/')return <AdminPremiumBannerPage supabase={supabase}/>;if(path==='/admin/gestao'||path==='/admin/gestao/')return <AdminPlatformPage supabase={supabase}/>;const event=parseEventRoute(path);if(event)return <EventRoute/>;const citySlug=parseCityHome(path);if(citySlug)return <CityHomePage citySlug={citySlug}/>;return <div className="app"/>}
