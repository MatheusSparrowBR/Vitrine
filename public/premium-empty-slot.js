(() => {
  const PLACEHOLDER_SRC = '/premium-ad-placeholder.svg'
  ...
  const render = (root = document) => {
    root.querySelectorAll?.('.empty-v2:not([data-vl-premium-placeholder])').forEach((slot) => {
      ...
      slot.innerHTML = `<a class="vl-premium-placeholder-link" href="${TARGET_HREF}" aria-label="Anuncie sua empresa na VitrineLocal"><img src="${PLACEHOLDER_SRC}" alt="Anuncie sua empresa aqui na VitrineLocal" loading="eager" decoding="async"></a>`
    })
  }
