...
test('rotas principais não exibem tela branca nem erro de runtime',async({page})=>{
 ...
 for(const route of routes){
  ...
  await expect(page.locator('.app-route-loading')).toHaveCount(0,{timeout:9000})
...
test('UX nova mantém a agenda visível quando não existem eventos',async({page})=>{
 ...
 const eventState=page.getByText(/Nenhum evento próximo(?: cadastrado)?\.?|getByText(/Nenhum evento próximo(?: cadastrado)?\.?|A agenda está tranquila por enquanto\.|Não foi possível carregar os eventos\.|Carregando agenda…/)|getByText(/Nenhum evento próximo(?: cadastrado)?\.?|A agenda está tranquila por enquanto\.|Não foi possível carregar os eventos\.|Carregando agenda…/)
...
test('home não bloqueia categorias e agenda quando uma fonte de dados falha',async({page})=>{
 ...
 const eventState=page.getByText(/Nenhum evento próximo(?: cadastrado)?\.?|getByText(/Nenhum evento próximo(?: cadastrado)?\.?|A agenda está tranquila por enquanto\.|Não foi possível carregar os eventos\.|Carregando agenda…/)|getByText(/Nenhum evento próximo(?: cadastrado)?\.?|A agenda está tranquila por enquanto\.|Não foi possível carregar os eventos\.|Carregando agenda…/)
 const eventCardsLocator=page.locator('.lvp-event')
 await expect(eventState.or(eventCardsLocator.first())).toBeVisible({timeout:9000})
 const eventCards=await eventCardsLocator.count()
 const emptyState=await eventState.count()
