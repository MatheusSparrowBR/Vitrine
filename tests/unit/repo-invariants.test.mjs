...
test('segredos de servidor nao aparecem no frontend',()=>{for(const file of fs.readdirSync(path.join(root,'src')).filter(x=>x.endsWith('.jsx')||x.endsWith('.js'))){const src=read(`src/${file}`);assert.doesNotMatch(src,/STRIPE_SECRET_KEY|SUPABASE_SERVICE_ROLE_KEY|SUPABASE_SECRET_KEY/)}})
