...
test('fluxos administrativos e migrations críticas estão versionados',()=>{
 ...
 assert.ok(fs.existsSync(path.join(srcDir,'BusinessRegistrationPage.jsx')))
 assert.ok(fs.existsSync(path.join(srcDir,'AdminEventsPanel.jsx')))
 assert.ok(fs.existsSync(path.join(migrationDir,'20260909131445_enforce_promotion_review_workflow.sql')))
 assert.ok(fs.existsSync(path.join(migrationDir,'20260909174253_create_city_events.sql')))
 assert.ok(fs.existsSync(path.join(migrationDir,'20260910011000_harden_business_insert_moderation.sql')))
 assert.ok(fs.existsSync(path.join(migrationDir,'20260910011100_harden_billing_event_privileges.sql')))
})
