...
test('entrypoint usa somente a nova árvore de frontend',()=>{
 ...
 for(const token of ['CityHomePage','BusinessesPage','PromotionsPage','BusinessProfilePage','AccountPage','AdminHomePage','AdminBusinessesPage','BusinessRegistrationPage','AuthPage','PasswordUpdatePage','BillingPlansPage','PrivacyPage','TermsPage','MercadoPagoCheckoutPage'])assert.match(app,new RegExp(token))
 assert.match(app,/path==='\/admin\/empresas'/)
 assert.match(app,/path==='\/checkout\/mercadopago'/)
...
test('billing webhook Mercado Pago usa assinatura e idempotencia',()=>{const src=read('supabase/functions/mercadopago-webhook/index.ts');for(const value of ['MERCADOPAGO_WEBHOOK_SECRET','x-signature','x-request-id','HMAC','billing_events','provider_event_id','processed_at'])assert.match(src,new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));assert.doesNotMatch(src,/stripe-signature|STRIPE_SECRET_KEY/)})
