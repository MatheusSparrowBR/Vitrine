...
test('desempenho e estatísticas avançadas são recursos separados sem duplicar gráficos',()=>{
 ...
 assert.match(page,/O que os dados avançados acrescentam/)
 assert.doesNotMatch(page,/ca-advanced-card[sS]*Tendência diária/)
 assert.doesNotMatch(page,/ca-advanced-card[sS]*Canais de contato/)
})
