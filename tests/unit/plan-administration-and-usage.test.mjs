...
test('meu plano diferencia capacidade armazenada e quota mensal',()=>{
 ...
 for(const value of ["LABELS={photos:'Mídias',items:'Produtos e serviços',promotions:'Promoções'",'function UsageCard(','CAPACIDADE ATUAL','QUOTA MENSAL','Math.round(percent)','const unlimited=limit<0','unlimited?\'∞\'','SEM LIMITE PARA TESTES'])assert.ok(plan.includes(value),`Meu plano precisa conter ${value}`)
})
