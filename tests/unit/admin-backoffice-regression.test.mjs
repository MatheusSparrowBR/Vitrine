...
test('admin platform exposes the plan editor and business-plan assignment flows',()=>{
 ...
 assert.ok(shell.includes("['plans','Planos', '" ) || shell.includes("/admin/gestao?tab=plans"))
})

test('enterprise moderation writes review metadata through the database trigger',()=>{
 const fn=read('supabase/migrations/20260911130500_admin_backoffice_hardening.sql')
 assert.match(fn,/CREATE OR REPLACE FUNCTION public\.protect_business_moderation_fields\(\)/)
 assert.match(fn,/new\.reviewed_at := now\(\)/)
 assert.match(fn,/new\.reviewed_by := \(select auth\.uid\(\)\)/)
 assert.match(fn,/new\.status is distinct from old\.status/)
 assert.match(fn,/new\.rejection_reason is distinct from old\.rejection_reason/)
})
