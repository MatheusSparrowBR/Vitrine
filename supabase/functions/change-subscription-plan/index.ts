...
Deno.serve(async req=>{
 ...
 try{
  ...
  const {data:business,error:businessError}=await db.from('businesses').select('id,name,owner_id').eq('id',businessId).eq('owner_id',user.id).maybeSingle();if(businessError)return response({error:'Não foi possível validar a empresa.'},500);if(!business)return response({error:'Empresa não encontrada ou sem permissão.'},403)
  const {data:current,error:currentError}=await db.from('subscriptions').select('id,plan_id,status,provider_subscription_id,provider_customer_id,billing_interval').eq('business_id',business.id).eq('user_id',user.id).eq('provider','mercadopago').in('status',['incomplete','active','trialing','past_due','unpaid','paused']).order('created_at',{ascending:false}).limit(1).maybeSingle();if(currentError)return response({error:'Não foi possível carregar a assinatura atual.'},500);if(!current?.provider_subscription_id)return response({error:'Nenhuma assinatura Mercado Pago foi encontrada para esta empresa.'},404)
  if(current.status==='incomplete')return response({error:'Conclua o pagamento da assinatura atual antes de alterar o plano.'},409)
  const {data:target,error:targetError}=await db.from('plans').select('id,code,name,price_monthly,price_yearly,payment_provider').eq('code',targetCode).eq('active',true).maybeSingle();if(targetError)return response({error:'Não foi possível carregar o plano de destino.'},500);if(!target)return response({error:'Plano de destino não encontrado.'},404)
  if(targetCode==='free'){
    ...
    const{error}=await db.from('subscriptions').update({status:'canceled',ends_at:new Date().toISOString(),current_period_end:new Date().toISOString(),cancel_at_period_end:false,scheduled_plan_id:null,scheduled_billing_interval:null,scheduled_change_at:null,updated_at:new Date().toISOString()}).eq('id',current.id);if(error)throw error
    return response({mode:'canceled_at_period_end',effective_at:new Date().toISOString(),provider:'mercadopago'})
  }
  const amount=Number(interval==='yearly'?target.price_yearly:target.price_monthly);if(!Number.isFinite(amount)||amount<=0)return response({error:'Preço do plano inválido.'},500)
  if(target.id===current.plan_id&&interval===current.billing_interval)return response({mode:'noop',provider:'mercadopago'})
  const updated=await mpRequest(`/preapproval/${encodeURIComponent(current.provider_subscription_id)}`,{method:'PUT',body:JSON.stringify({reason:`VitrineLocal ${target.name}`,external_reference:[business.id,target.id,interval,user.id].join('|'),auto_recurring:{frequency:interval==='yearly'?12:1,frequency_type:'months',transaction_amount:amount,currency_id:'BRL'}})})
