...
Deno.serve(async req=>{
 ...
 const businessId=String(body?.business_id||''),targetCode=String(body?.plan_code||''),interval=body?.interval==='yearly'?'yearly':'monthly'
 if(!businessId||!['free','pro','premium'].includes(targetCode))return json({error:'Empresa, plano e intervalo são obrigatórios.'},400)
 ...
 if(targetCode==='free'){
   const updated:any=await stripe.subscriptions.update(subscription.id,{cancel_at_period_end:true,metadata:{business_id:business.id,user_id:user.id,plan_id:currentPlan?.id||record.plan_id,plan_code:currentCode,billing_interval:record.billing_interval||interval}})
 ...
 await stripe.subscriptionSchedules.update(schedule.id,{end_behavior:'release',phases:[{start_date:currentStart,end_date:subscription.current_period_end,items:[{price:item.price.id,quantity:item.quantity||1}],metadata:{plan_id:String(currentPlan?.id||record.plan_id),plan_code:String(currentCode),billing_interval:currentInterval}},{items:[{price:targetPriceId,quantity:1}],metadata:{plan_id:String(target.id),plan_code:String(target.code),billing_interval:interval}}]} as any)
