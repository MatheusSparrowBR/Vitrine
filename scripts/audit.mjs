...
for(const p of required)(existsSync(join(root,p))?ok:fail).push((existsSync(join(root,p))?'':'Missing ')+p)

const app=read('src/app-entry.jsx')
if(/path===['"]\/preview-lancamento['"]/.test(app)&&/route\.kind==='home'&&route\.citySlug==='laguna'/.test(app)&&/LaunchHomePreview/.test(app))ok.push('Laguna launch homepage is wired in the main router')
...
else warn.push('CI dependency audit not configured')
...
function walk(dir){
 for(const name of readdirSync(dir)){
  ...
  else if(s.size<2_000_000 && /\.(js|jsx|mjs|cjs|json|yml|yaml|html|css|md|txt|conf)$/.test(name)){
   const content=readFileSync(p,'utf8')
   if(forbidden.test(content))fail.push('Forbidden Vercel reference in '+rel)
   if(/SUPABASE_SERVICE_ROLE_KEY\s*=\s*['"][^'"]+['"]|service_role\s*[:=]\s*['"][^'"]{10,}/i.test(content)&&!rel.startsWith('docs/'))warn.push('Review possible secret-like service-role reference in '+rel)
  }
 }
}
...
for(const s of ['audit:static','test','build','test:e2e']) if(!pkg.scripts?.[s] && s!=='audit:static') fail.push('Missing package script: '+s)
